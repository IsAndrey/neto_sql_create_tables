...
CREATE TABLE IF NOT EXISTS artists_genres (
    ...
    CONSTRAINT artist_genre UNIQUE (artist_id, genre_id)
);
...
CREATE TABLE IF NOT EXISTS emploees (
    id SERIAL,
    em_level SMALLINT NOT NULL DEFAULT 0,
    PRIMARY KEY (id, em_level),
    id_department INT REFERENCES departments(id) NOT NULL,
    chief_id INT,
    chief_level INT,
    CONSTRAINT fk_chief FOREIGN KEY (chief_id, chief_level) REFERENCES emploees(id, em_level) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT chek_chief_id CHECK(chief_id IS NULL OR chief_id != id),
    CONSTRAINT check_chief_level CHECK(chief_level IS NULL OR chief_level > em_level)
);

/*
 * ограничения призваны исключить ситуацию, когда Иванов босс Сидорова, Сидоров босс Петрова, а Петров босс Иванова
*/